...
//                "--file=/public/log,/public/config,/public/data",
        ...
        try {
            ...
            Assert.assertEquals("有三个命令行参数", 4, line.getOptions().length);
            ...
            Assert.assertTrue("命令行参数中包含 h", line.hasOption("h"));

            Assert.assertEquals("Option n 的参数值等于 no one", "no one", line.getOptionValue("n"));

            // 没有直接获取到 Option 的方法，这里按照解析的顺序获取到 Option f
            Option optionF = line.getOptions()[1];
            Assert.assertEquals("Option f 有三个参数值", 3, optionF.getValues().length);
            Assert.assertEquals("Option f 的参数值类型为 String", String.class, optionF.getType());
            Assert.assertEquals("Option f 第一个参数值为 /public/log ", "/public/log", optionF.getValue(0));
